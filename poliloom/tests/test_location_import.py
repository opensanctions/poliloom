...
class TestLocationImport:
    ...
    def test_import_all_locations_success(self, test_session, sample_locations_data):
        ...
        with patch.object(WikidataClient, 'get_all_locations', return_value=sample_locations_data):
    ...
    def test_import_all_locations_database_error(self, test_session, sample_locations_data):
        ...
        with patch.object(WikidataClient, 'get_all_locations', return_value=sample_locations_data):
            ...
            with patch('poliloom.services.import_service.SessionLocal', return_value=broken_session):
                import_service = ImportService()
                count = import_service.import_all_locations()
    ...
    def test_import_all_locations_batch_commit(self, test_session):
        ...
        for i in range(50):  # Use smaller number for testing
            large_dataset.append({
                "wikidata_id": f"Q{i+100000}",
                "name": f"Location {i+1}"
            })

        with patch.object(WikidataClient, 'get_all_locations', return_value=large_dataset):
