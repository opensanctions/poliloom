...
class TestImportService:
    ...
    def test_import_politician_by_id_success(
        self, import_service, test_session, sample_politician_data, mock_wikidata_client
    ):
        ...
        politician = (
            test_session.query(Politician).filter_by(wikidata_id="Q123456").first()
        )
        ...
        properties = (
            test_session.query(Property).filter_by(politician_id=politician.id).all()
        )
        assert (
            len(properties) == 1
        )  # Only BirthDate (birthplace uses BornAt relationship, citizenship uses HasCitizenship)
        ...
        source = (
            test_session.query(Source)
            .filter_by(url="https://en.wikipedia.org/wiki/John_Doe")
            .first()
        )
        assert source is not None
        assert source in politician.sources

    def test_import_politician_already_exists(
        self,
        import_service,
        test_session,
        ...
        politicians = (
            test_session.query(Politician).filter_by(wikidata_id="Q123456").all()
        )
        assert len(politicians) == 1

    def test_import_politician_wikidata_error(
        self, import_service, test_session, mock_wikidata_client
    ):
    ...
    def test_import_politician_database_error(
        self, import_service, test_session, sample_politician_data, mock_wikidata_client
    ):
    ...
    def test_create_politician_with_minimal_data(self, import_service, test_session):
        ...
        minimal_data = {"wikidata_id": "Q123456", "name": "John Doe"}
    ...
    def test_create_properties_skips_empty_values(
        ...
        properties = [
            ...
            {"type": "DeathDate", "value": ""},  # Empty value
            {
                "type": "BirthDate",
                "value": None,
            },  # None value (duplicate type for testing)
        ]
    ...
    def test_link_to_existing_positions_only(
        self, import_service, test_session, sample_politician, sample_position
    ):
        ...
        import_service._link_to_existing_positions(
            test_session, sample_politician, positions
        )
        test_session.flush()
    ...
    def test_holds_position_dates_with_existing_position(
        self, import_service, test_session, sample_politician_data, mock_wikidata_client
    ):
    ...
    def test_create_citizenships_with_existing_country(
        ...
        citizenship = (
            test_session.query(HasCitizenship)
            .filter_by(
                politician_id=sample_politician.id, country_id=existing_country.id
            )
            .first()
        )
        assert citizenship is not None

    def test_create_citizenships_prevents_duplicates(
        self, import_service, test_session, sample_politician
    ):
        ...
        existing_citizenship = HasCitizenship(
            politician_id=sample_politician.id, country_id=country.id
        )
