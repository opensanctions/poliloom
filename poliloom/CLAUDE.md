...
- **Politician**
  ...
  - wikipedia_en_url (String, URL to English Wikipedia page)
  - wikipedia_local_url (String, URL to local language Wikipedia page)
...
- **Property**
  ...
  - source_id (Foreign Key to Source.id, many-to-many relationship managed through association table if multiple sources contribute to one property)
  - is_new (Boolean, True if newly extracted and unconfirmed)
  - confirmed_by (String, ID of user who confirmed, Null if unconfirmed)
  - confirmed_at (Datetime, Null if unconfirmed)
  - original_wikidata_value (String, current value in Wikidata for comparison)
  - conflict_resolved (Boolean, indicates if conflicts with Wikidata have been addressed)
- **Position**
...
- **HoldsPosition** (Many-to-many relationship entity)
  ...
  - source_id (Foreign Key to Source.id, many-to-many relationship if multiple sources contribute to one position held)
  - is_new (Boolean, True if newly extracted and unconfirmed)
  - confirmed_by (String, ID of user who confirmed, Null if unconfirmed)
  - confirmed_at (Datetime, Null if unconfirmed)
  - original_wikidata_start_date (String, current start date in Wikidata)
  - original_wikidata_end_date (String, current end date in Wikidata)
  - conflict_resolved (Boolean, indicates if conflicts with Wikidata have been addressed)
- **Association Tables:**
