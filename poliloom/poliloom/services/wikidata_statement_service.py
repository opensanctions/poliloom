"""Wikidata statement creation service for pushing confirmed evaluations to Wikidata."""
...
class WikidataStatementService:
    """Service for creating Wikidata statements from confirmed evaluations."""

    def __init__(self):
        """Initialize the Wikidata statement service."""
        ...
        self.base_url = wikidata_api_root
    ...
    async def create_statement(
        ...
        url = f"{self.base_url}/entities/items/{entity_id}/statements"
    ...
    async def create_birth_date_statement(
        ...
        return await self.create_statement(
            politician_id,
            "P569",  # Date of birth
            wikidata_value,
    ...
    async def push_confirmed_evaluation(
        ...
        try:
            if isinstance(evaluation, PropertyEvaluation):
                return await self._push_property_evaluation(evaluation, jwt_token, db)
            elif isinstance(evaluation, PositionEvaluation):
                return await self._push_position_evaluation(evaluation, jwt_token, db)
            elif isinstance(evaluation, BirthplaceEvaluation):
                return await self._push_birthplace_evaluation(evaluation, jwt_token, db)
            else:
    ...
    async def _push_property_evaluation(
        ...
        if prop.type == "date_of_birth":
            source_url = self._get_source_url(prop, db)
            statement_id = await self.create_birth_date_statement(
    ...
    async def _push_position_evaluation(
        ...
        source_url = self._get_source_url(position, db)
        statement_id = await self.create_position_held_statement(
    ...
    async def _push_birthplace_evaluation(
        ...
        source_url = self._get_source_url(birthplace, db)
        statement_id = await self.create_birthplace_statement(
    ...
    def _get_source_url(self, entity: Any, db: Session) -> str:
        """
...
_service_instance: Optional[WikidataStatementService] = None


def get_wikidata_statement_service() -> WikidataStatementService:
    """Get or create the global Wikidata statement service instance."""
    global _service_instance
    if _service_instance is None:
        _service_instance = WikidataStatementService()
